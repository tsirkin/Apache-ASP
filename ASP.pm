...
sub handler {
    ...
    if($self->{filter} || ($status == 500) || ( $r->isa('Apache::ASP::CGI')) || 
       ### Should we really destroy the env in PSGI ?Or maybe there a better way?
       ( $r->isa('Apache::ASP::PSGI'))) {
	$self->DESTROY();
    }
...
=item $VERSION = 2.63; $DATE="2013/09/20"
...
Copyright (c) 1998-2012, Josh Chamas, Chamas Enterprises Inc. 
